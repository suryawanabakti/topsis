...
<div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 max-w-2xl mx-auto">
    <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800">Form Edit Sub Kriteria</h3>
    </div>
    <div class="p-6">
        <form action="{{ route('sub_kriteria.update', $sub_kriteria->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                ...
                <select name="kriteria_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    @foreach($kriterias as $k)
                        <option value="{{ $k->id }}" {{ old('kriteria_id', $sub_kriteria->kriteria_id) == $k->id ? 'selected' : '' }}>{{ $k->kode }} - {{ $k->nama_kriteria }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="nama_sub" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('nama_sub', $sub_kriteria->nama_sub) }}">
            ...
            <div>
                ...
                <input type="number" name="nilai" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" value="{{ old('nilai', $sub_kriteria->nilai) }}">
